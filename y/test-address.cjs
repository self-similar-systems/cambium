...
const registry=JSON.parse(regMatch[1]);eq(registry.source,'w/display/y tree');eq(new Set(registry.mounts.map(m=>m.address)),new Set(['','w','y']));
const mountById=Object.fromEntries(registry.mounts.map(m=>[m.interlocutor,m.address]));
eq(mountById,{'organism:philosophy':'','organism:crawlerbait':'w','organism:papers':'y'});
